...
> Fresh start in `t1d_bot_v2/` on `main` branch. Expanding from structured ISPAD-only PDFs to a diverse, multilingual, multi-format corpus with a repeatable ingestion pipeline. Current codebase preserved in `v1` branch; `t1d_bot_v2/` promoted to repo root once v1 logic is no longer needed.
...
| 17 | Project Start | **Fresh start** in `t1d_bot_v2/` on `main` branch. Current codebase preserved in `v1` branch. Port ~300 lines from v1: PyMuPDF table extraction patterns, checkpoint/resume logic, dosage/recommendation regex patterns. Once v2 is verified, old root-level code removed and `t1d_bot_v2/` contents promoted to repo root |
...
| 22 | LLM Abstraction | `LLMClient` seam in `src/llm/`. `OllamaAdapter` is the concrete implementation now; `GeminiAdapter` / `OpenAIAdapter` are future ~20-line additions. All LLM-calling modules (Chunker, MetadataGenerator, Generator) import `LLMClient` — no raw `ollama.chat()` calls outside `src/llm/` |
...
Future adapters (`GeminiAdapter`, `OpenAIAdapter`) are ~20-line implementations of `LLMClient.chat()`. Adding a new provider requires no changes outside `src/llm/`.
...
```python
# t1d_bot_v2/src/pipeline/flows.py
...
2. On main: create t1d_bot_v2/ directory → all v2 development here
3. Once v2 verified end-to-end:
   ├── Remove old root-level src/, app.py, notebooks/
   ├── Promote t1d_bot_v2/ contents to repo root
   └── t1d_bot_v2/ becomes the new repo state
```
...
Step 1: Project Scaffold
  ├── Create t1d_bot_v2/ directory + subdirectories
...
Step 2: LLMClient Seam (build before all pipeline modules)
  ...
  └── Verify: OllamaAdapter.chat() returns a string for a test prompt
...
Step 11: Promote v2 to Root
  ...
  ├── Move t1d_bot_v2/ contents to repo root
...
6. **Additional LLM Adapters**: `GeminiAdapter` / `OpenAIAdapter` — each ~20-line implementation of `LLMClient.chat()`. No changes outside `src/llm/`

---

## File Changes Summary

All files are **NEW** in `t1d_bot_v2/`. No files are modified or deleted from v1 during development — v1 is preserved intact in the `v1` branch.
